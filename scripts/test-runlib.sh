#!/usr/bin/env bash
# Hermetic tests for scripts/runlib.sh (Issue #54).
#
...
set -euo pipefail
if [[ "${1:-}" == "metadata" ]]; then
  printf '%s\n' '{"packages":[{"name":"neat-ai-refinery","version":"9.9.9"}]}'
  exit 0
fi
...
printf 'fake\n' >"${HOME}/.cargo/bin/neat_ai_refinery"
chmod +x "${HOME}/.cargo/bin/neat_ai_refinery"
printf '9.9.9\n' >"${HOME}/.cargo/bin/.neat_ai_refinery.version"
...
assert_eq "already-installed stdout is the CLI path" \
  "${HOME}/.cargo/bin/neat_ai_refinery" "${OUT}"
assert_eq "already-installed names the version on stderr" "0" \
  "$(grep -q '\[neat-ai-refinery\] already installed v9.9.9' "${WORK_DIR}/already.err"; echo $?)"

echo ""
echo "=== missing stamp is not treated as already installed ==="
rm -f "${HOME}/.cargo/bin/.neat_ai_refinery.version"
